...
describe('GoalAllocationRepository (Integration)', () => {
  beforeAll(async () => {
    await connect();
  });
  ...
  it('should create and retrieve a document', async () => {
    ...
    try {
       const doc = await GoalAllocationRepository.create({});
       expect(doc._id).toBeDefined();
       
       const found = await GoalAllocationRepository.findById(doc._id);
